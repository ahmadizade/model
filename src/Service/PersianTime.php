...
class PersianTime extends Controller
{
    public function DiffTimeAction($date1,$date2,$time1,$time2,$type='min'){
        ...
        $result += (525600 * ( $year2 - $year1));
        $result += (17280 * ($month2 - $month1)) ;
        $result += (1440 * ($day2 - $day1)) ;
        $result += (60 * ($hour2 - $hour1)) ;
        $result +=  ($min2 - $min1) ;
    ...
    public function LifeTimeAction($created_date,$today){
        list($year,$month,$day)=explode('/',$created_date);
...
return $content ;
    ...
    public function RemainTimeAction($date,$now){
    ...
    public function NowAction($pattern,$format='fa')
    {
        /*      $timezone = 0;//برای 3:30 عدد 12600 و برای 4:30 عدد 16200 را تنظیم کنید
                $now = date("Y-m-d", time()+$timezone);
                $created_time = date("H:i:s", time()+$timezone);
                list($year, $month, $day) = explode('-', $now);
                list($hour, $minute, $second) = explode(':', $created_time);
                $timestamp = mktime($hour, $minute, $second, $month, $day, $year);*/
        $timestamp=time();

        $created_time = $this->jdate($pattern,$timestamp,'','Asia/Tehran',$format);
        return new Response($created_time);
    }
    public function ConvertDate($date){
        list($year, $month, $day) = explode('-', $date);
        $timestamp = mktime('', '', '', $month, $day, $year);

        $created_time = $this->jdate("Y/m/d",$timestamp);
        return new Response($created_time);
    }
